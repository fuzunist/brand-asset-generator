...
const generateSocialMediaKit = async (logoBuffer, primaryColor, secondaryColor, brandName, outputDir = null) => {
    ...
    for (const platform in specs) {
        for (const type in specs[platform]) {
            ...
            const resizedLogoBuffer = await sharp(logoBuffer).resize({ width: Math.floor(width * 0.8) }).toBuffer();
            ...
            if (outputDir) {
                const filePath = path.join(outputDir, `${platform}_${type}_white.${format}`);
                fs.writeFileSync(filePath, whiteBgImage);
                outputPromises.push(filePath);
            } else {
            ...
            if (outputDir) {
                const filePath = path.join(outputDir, `${platform}_${type}_primary.${format}`);
                fs.writeFileSync(filePath, primaryBgImage);
                outputPromises.push(filePath);
            } else {
            ...
            if (outputDir) {
                const filePath = path.join(outputDir, `${platform}_${type}_secondary.${format}`);
                fs.writeFileSync(filePath, secondaryBgImage);
                outputPromises.push(filePath);
            } else {
            ...
            if (type === 'cover') {
                ...
                const leftAlignedImage = await sharp({ create: { width, height, channels: 4, background: primaryRgb } })
                    .composite([{ input: resizedLogoBuffer, gravity: 'west' }])
                    .toFormat(format)
                    .toBuffer();

                if (outputDir) {
                    const filePath = path.join(outputDir, `${platform}_${type}_primary_left.${format}`);
                    fs.writeFileSync(filePath, leftAlignedImage);
                    outputPromises.push(filePath);
                } else {
                ...
                if (outputDir) {
                    const filePath = path.join(outputDir, `${platform}_${type}_gradient.${format}`);
                    fs.writeFileSync(filePath, gradientImage);
                    outputPromises.push(filePath);
                } else {
