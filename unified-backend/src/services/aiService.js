// unified-backend/src/services/aiService.js
const OpenAI = require('openai');
const { knex: db } = require('../database'); // Use the centralized knex instance
const logger = require('../utils/logger');

// ... (rest of the file content from content-calendar-module/backend/src/services/aiService.js)
// We will paste the original content here and only change the require path for the database.
class AIService {
  constructor() {
    if (process.env.OPENAI_API_KEY) {
        ...
        this.openai = null;
        logger.warn('OpenAI API key is not configured. AI-related features will be disabled.');
    }
  ...
  async generateContentPrompts(eventName, userIndustry, eventDescription = null) {
    if (!this.openai) {
        logger.error('Cannot generate AI prompts because OpenAI API key is not configured.');
